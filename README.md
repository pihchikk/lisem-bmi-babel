...
В репозитории есть один полностью готовый к запуску реальный датасет —
`VNIIMZ_20m` (`meta/Lisem/vniimz_20m/`). Собственный runfile для него не
нужен, скачивать ничего не надо — но работает это **при запуске из чекаута
исходников**: в собранное колесо данные не кладутся, см. раздел про каталог
с данными ниже. Что это за датасет:
...
Датасет лежит в репозитории, в `meta/Lisem/vniimz_20m/`, и **в колесо не
попадает** — собранный `.whl` содержит только код. Поэтому
`default_scenario_runfile()` работает при запуске из чекаута исходников, а из
установленного колеса выбросит `FileNotFoundError` (в сообщении будут оба
пути, где он искал).

Если запускаете из установленного пакета — передавайте `initialize()` путь к
своему runfile напрямую:

```python
model.initialize("/абс/путь/к/своему.run")
```

Вернуть данные в колесо можно, раскомментировав `install_subdir` в
`meson.build` — там же написано, как именно.
